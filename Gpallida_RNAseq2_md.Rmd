...
v1 <- make_volcanoplot(trtm = c("water", "solA"), tp = 8, log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 8h, water vs. solA", method = "treatment", xsize = 2.5)
v2 <- make_volcanoplot(trtm = c("water", "solA"), tp = 24, log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 24h, water vs. solA", method = "treatment", xsize = 3)
v3 <- make_volcanoplot(trtm = c("water", "solA"), tp = 48, log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 48h, water vs. solA", method = "treatment", xsize = 10)
...
v4 <- make_volcanoplot(trtm = c("water", "PRD"), tp = 8, treatment2 = "PRD", log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 8h, water vs. PRD", method = "treatment", xsize = 8)
v5 <- make_volcanoplot(trtm = c("water", "PRD"), tp = 24, treatment2 = "PRD", log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 24h, water vs. PRD", method = "treatment", xsize = 8)
v6 <- make_volcanoplot(trtm = c("water", "PRD"), tp = 48, treatment2 = "PRD", log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 48h, water vs. PRD", method = "treatment", xsize = 10)
...
v7 <- make_volcanoplot(trtm = c("solA", "PRD"), tp = 8, ref_treatment = "solA", treatment2 = "PRD", log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 8h, solA vs. PRD", method = "treatment", xsize = 8)
v8 <- make_volcanoplot(trtm = c("solA", "PRD"), tp = 24, ref_treatment = "solA", treatment2 = "PRD", log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 24h, solA vs. PRD", method = "treatment", xsize = 8)
v9 <- make_volcanoplot(trtm = c("solA", "PRD"), tp = 48, ref_treatment = "solA", treatment2 = "PRD", log2FC_threshold = 0, FCcutoff_volcano = 1, padj_threshold = 0.01, ttl = "Timepoint 48h, solA vs. PRD", method = "treatment", xsize = 8)
...
solA_8h <- get_list_of_DEGs(trtm = c("water", "solA"), ref_treatment = "water", treatment2 = "solA", tp = 8, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")

solA_24h <- get_list_of_DEGs(trtm = c("water", "solA"), ref_treatment = "water", treatment2 = "solA", tp = 24, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")

solA_48h <- get_list_of_DEGs(trtm = c("water", "solA"), ref_treatment = "water", treatment2 = "solA", tp = 48, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")
...
Indeed, 24h and 48h overlap quite a bit (428 DEGs overlap), much more than either with the 8h timepoint (131 and 73, respectively).

Let's do the same thing for the PRD treated samples:

```{r DEGslist2, warning = FALSE, message = FALSE}
PRD_8h <- get_list_of_DEGs(trtm = c("water", "PRD"), ref_treatment = "water", treatment2 = "PRD", tp = 8, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")
PRD_24h <- get_list_of_DEGs(trtm = c("water", "PRD"), ref_treatment = "water", treatment2 = "PRD", tp = 24, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")
PRD_48h <- get_list_of_DEGs(trtm = c("water", "PRD"), ref_treatment = "water", treatment2 = "PRD", tp = 48, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")
...
PRD_vs_solA_8h <- get_list_of_DEGs(trtm = c("solA", "PRD"), ref_treatment = "solA", treatment2 = "PRD", tp = 8, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")
PRD_vs_solA_24h <- get_list_of_DEGs(trtm = c("solA", "PRD"), ref_treatment = "solA", treatment2 = "PRD", tp = 24, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")
PRD_vs_solA_48h <- get_list_of_DEGs(trtm = c("solA", "PRD"), ref_treatment = "solA", treatment2 = "PRD", tp = 48, log2FC_threshold = 0, padj_threshold = 0.05) %>% rownames_to_column("Geneid")
...
ud1$group <- factor(ud1$group, levels = c("solA_8h", "solA_48h","solA_24h", "PRD_48h", "PRD_8h", "PRD_24h"))
...
Interesting in this plot: PRD leads to many more DEGs than solA treatment, at all timepoints. For both treatments, most DEGs are at 24h. In PRD, there are more upregulated genes than downregulated, and for solA this is the other way around.
Furthermore, whereas there are 326 genes specifically differentially expressed in all timepoints of PRD treatment, only 3 are specific for all timepoints of solA treatment. Potentially, these 326 PRD-specific genes are upregulated by the compounds in root exudate that are not (related to) solA.
There are no genes are differentially expressed in all 6 treatments, but there are 116 genes that are differentially expressed in all treatments but solA_8h. This treatment also has the least amount of DEGs of all treatments.
For time-specific DEGs, there are 6, 105, and 44 genes DEGs in both treatments at timepoints 8, 24 and 48h, respectively. For sample-specific DEGs, there are 411, 862 and 234 for PRD and 20, 287 and 64 for solA, exclusively differentially expressed genes at timepoints 8, 24 and 48h, respectively.
...
So there are two genes that are putative homologues of dauer genes in the solA_8h sample. Let's do the same for the other samples and assemble all results in one table.
...
Interestingly, daf-9 is upregulated (log2foldchange 2.0) after 24h in solA, but not in PRD. daf-12 is slightly downregulated after 24h in PRD (log2foldchange -0.5). There are many more differentially expressed dauer-related genes, and we should have an overview for all samples to see what is happening. Therefore, a heatmap showing all dauer-related genes and whether they are differentially expressed, is made.
...
heatm <- heatm[-c(2:42,44:46,48,49,55,58,76,82,92,93,95:97,105,106,108:113,117:130),]
row.names(heatm)[2] <- "daf-37.1"
row.names(heatm)[3] <- "daf-37.2"

#arrange pathway labels
dauergenes2 <- row.names(heatm)
pathway <- c(rep("cGMP",10), rep("insulin", 19), rep("TGFB", 9), rep("DA_biosynthesis", 18))
